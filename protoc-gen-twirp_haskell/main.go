...
func (g *generator) Generate() *plugin.CodeGeneratorResponse {
	...
	for _, f := range g.protoFilesToGenerate() {
		...
		pbFileRelativePath := packageFileName(onlyBase(filePath(f))) + "PB.hs" // e.g. "ServicePB.rb"

		haskellCode := g.generateHaskellCode(f, pbFileRelativePath)
...
func (g *generator) generateHaskellCode(file *descriptor.FileDescriptorProto, pbFileRelativePath string) string {
	b := new(bytes.Buffer)
